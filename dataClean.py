...
date_pmc = pd.to_datetime(df['Date_premiere_mise_en_circulation'], errors='coerce')
df['Date_PMC_Float'] = date_pmc.apply(lambda x: x.timestamp() / 31536000 if pd.notnull(x) else np.nan)
...
print('Final data length', len(df))import numpy as np
import pandas as pd
pd.options.mode.chained_assignment = None
import matplotlib.pyplot as plt
import matplotlib
